...
interface AvatarUpdateWarningProps {
  onUpdate: () => void;
}

const AvatarUpdateWarning: React.FC<AvatarUpdateWarningProps> = ({ onUpdate }) => {
  return (
    <div className="fixed inset-0 z-[150] bg-black/90 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-300">
      <div className="bg-[#111111] w-full max-w-xs rounded-[2.5rem] p-8 space-y-6 border border-white/10 shadow-2xl text-center">
        ...
        <div className="space-y-3">
          <h4 className="text-xl font-black text-white uppercase tracking-tighter">Yêu cầu ảnh đại diện</h4>
          <p className="text-[11px] font-bold text-gray-500 leading-relaxed uppercase tracking-tight">
            Vui lòng cập nhật <span className="text-[#ff8c00]">ảnh đại diện</span> chính chủ để xác minh danh tính và kích hoạt các tính năng của hệ thống.
          </p>
        </div>
        ...
        >
          Cập nhật ảnh ngay <ChevronRight size={14} />
        </button>
      </div>
    </div>
  );
...
export default AvatarUpdateWarning;
